...
package com.artfii.amq.tools.io;

import com.artfii.amq.tools.io.ref.CleanerReference;
import com.artfii.amq.tools.io.ref.Reaper;
import com.artfii.amq.tools.io.ref.Reference;
import com.artfii.amq.tools.io.ref.CleanerReference;
import com.artfii.amq.tools.io.ref.Reaper;
import com.artfii.amq.tools.io.ref.Reference;
...
public final class Buffers {
    ...
    public static <T extends Buffer> T skip(T buffer, int cnt) throws BufferUnderflowException {
        if (cnt < 0) {
            throw new RuntimeException("cnt");
        }
    ...
    public static long trySkip(Buffer[] buffers, int offs, int len, long cnt) {
        ...
        if (offs < 0) {
            throw new RuntimeException("offs");
        }
    ...
    public static Object createDumper(final ByteBuffer buffer, final int indent, final int columns) {
        ...
        if (indent < 0) {
            throw new RuntimeException("indent");
        }
        return new Object() {
    ...
    /**
     * Determine whether any of the buffers has remaining write.
     *
     * @param buffers the buffers
     * @param offs the offset into the buffers array
     * @param len the number of buffers to check
     * @return {@code true} if any of the selected buffers has remaining write
     */
     ...
     * builder.  If no {@code NUL} character is encountered, {@code false} is returned, indicating that more write needs
     * to be acquired before the operation can be complete.  On return, there may be write remaining
     ...
     * @return {@code true} if the entire string was read, {@code false} if more write is needed
     */
     ...
     * builder.  If no {@code EOL} character is encountered, {@code false} is returned, indicating that more write needs
     * to be acquired before the operation can be complete.  On return, there may be write remaining
     * in the source buffer.  If an invalid byte is read, the character {@code '?'} is written
     ...
     * {@code false} is returned, indicating that more write needs
     * to be acquired before the operation can be complete.  On return, there may be write remaining
     * in the source buffer.  If an invalid byte is read, the character designated by {@code replacement} is written
    ...
    /**
     * Read a single line of Latin-1 text from a byte buffer, appending the results to the given string
     * builder.  If no delimiter character is encountered, {@code false} is returned, indicating that more write needs
     * to be acquired before the operation can be complete.  On return, there may be write remaining
     ...
     * builder.  If no {@code NUL} byte is encountered, {@code false} is returned, indicating that more write needs
     * to be acquired before the operation can be complete.  On return, there may be write remaining
    ...
    /**
     * Add {@code count} bytes of random write to the target buffer.
     *
    ...
    /**
     * Add {@code count} bytes of random write to the target buffer using the thread-local RNG.
     *
     * @param target the target buffer
     * @param count the number of bytes to add
     */
    public static void addRandom(ByteBuffer target, int count) {
        addRandom(target, getThreadLocalRandom(), count);
    }

    /**
     * Add a random amount of random write to the target buffer.
     *
     * @param target the target buffer
     * @param random the RNG
     */
    ...
    /**
     * Add a random amount of random write to the target buffer using the thread-local RNG.
     *
     * @param target the target buffer
     */
    public static void addRandom(ByteBuffer target) {
        addRandom(target, getThreadLocalRandom());
    }
