...
import com.artfii.amq.tools.io.cpu.CacheInfo;
import com.artfii.amq.tools.io.cpu.CacheInfo;
...
public abstract class ByteBufferPool {
    ...
    /**
     * The size (8192) of medium buffers.
     */
     ...
     * to be separately repooled, leading to likely write corruption.
     *
    ...
    public void toFree(ByteBuffer buffer){
        this.free(buffer);
    }
     ...
     */
    public <T, U, E extends Exception> void acceptWithCacheEx(int cacheSize, BiConsumer<T, U> consumer, T param1, U param2) throws E {
     ...
     * @throws E if the nested action threw an exception
     */
    public <T, U, R, E extends Exception> R applyWithCacheEx(int cacheSize, BiFunction<T, U, R> function, T param1, U param2) throws E {
        final ThreadLocal<Cache> threadLocalCache = this.threadLocalCache;
        final Cache parent = threadLocalCache.get();
        final Cache cache;
        if (cacheSize == 0) {
