...
public @interface Url {
    ...
    enum Scan {
        ...
        Scan() {
            try {
                classList = AnnotationDetector.scanClassPath("com.artfii.amq.admin","com.artfii.amq.tester") // or: scanFiles(File... files)
...
//                        .filter((File dir, String name) -> !dir.getName().startsWith("com.artfii.amq.tools")) // optional, default all *.class files
                        .collect(s -> s.getType());
            } catch (IOException e) {
                e.printStackTrace();
            }
